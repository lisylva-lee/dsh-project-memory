...
function packageAssetsRoot() {
	// Bundled into lib/index.js: "../assets/" resolves to the package root;
	// running from src/ (dev/tests) needs "../../assets/". Probe both.
	const candidates = [
...
function defaultSkillDir(home = process.env.DSH_HOME ?? join(homedir(), ".dsh")) {
	return join(home, "skills", "project-memory");
}
/** Resolve the skill root: the user skill dir when present, else the bundled assets. */
function resolveSkillDir(home = process.env.DSH_HOME ?? join(homedir(), ".dsh")) {
...
function resolveTemplateDir(home = process.env.DSH_HOME ?? join(homedir(), ".dsh")) {
	const user = defaultSkillDir(home);
...
function loadSkillContent(home = process.env.DSH_HOME ?? join(homedir(), ".dsh")) {
...
function configPath(home = process.env.DSH_HOME ?? join(homedir(), ".dsh")) {
